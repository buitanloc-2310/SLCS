...
function __req(id){
  ...
  const fn=__mods[id]; if(!fn) throw new Error('QR module not found: '+id);
  const module={exports:{}}; __cache[id]=module;
  const localReq=(x)=>{ if(x.startsWith('./')) return __req('./'+x.replace(/^\.\//,'')); return __req(x); };
  fn(module,module.exports,localReq); return module.exports;
}
const QRCode=__req('./index');
const QRErrorCorrectLevel=__req('./QRErrorCorrectLevel');
function toCanvas(canvas,text,opts={}){
  const qr=new QRCode(-1,QRErrorCorrectLevel.M); qr.addData(String(text||'')); qr.make();
  const n=qr.getModuleCount(), margin=Number.isFinite(opts.margin)?opts.margin:4, target=opts.width||240;
  const cells=n+margin*2, scale=Math.max(1,Math.floor(target/cells)), size=cells*scale;
